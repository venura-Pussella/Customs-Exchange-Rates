...
class CosmosObjects:
    ...
    def getCosmosClient(cls) -> CosmosClient:
        ...
        if CosmosObjects.__cosmosClient == None:
            ...
            logging.info("Azure Cosmos client created using given URL and key")
    ...
    def getCosmosDatabase(cls) -> AzureCosmosDatabase.DatabaseProxy:
        ...
        if CosmosObjects.__cosmosDatabase == None:
            try:
                ...
                logging.info(f"Cosmos Database created or returned: {CosmosObjects.__cosmosDatabase.id}")

            except AzureCosmosExceptions.CosmosHttpResponseError as e:
                logging.error("AzureCosmosExceptions.CosmosHttpResponseError raised in getCosmosDatabase() ")
                logging.error(e)
    ...
    def getCosmosContainer(cls) -> AzureCosmosContainer.ContainerProxy:
        ...
        if CosmosObjects.__cosmosContainer == None:
            try:
                ...
                logging.info(f"Cosmos Container created or returned: {CosmosObjects.__cosmosContainer.id}")

            except AzureCosmosExceptions.CosmosHttpResponseError as e:
                logging.error("AzureCosmosExceptions.CosmosHttpResponseError raised in getCosmosContainer() ")
                logging.error(e)

        return CosmosObjects.__cosmosContainer
    
    @classmethod
    def create_item_in_cosmos_container(cls, item: dict):
        container = cls.getCosmosContainer()
